...
attribution line. Nothing here uses the network.
"""
...
class Store:
    ...
    def cn_bundled(self, digits: str) -> bool:
        return any(digits.startswith(p) or p.startswith(digits) for p in
                   ("25", "28", "31", "72", "73", "76", "2601", "2716"))
...
def _scope_version(s: Store) -> str:
    m = s.annex["meta"]
    return (f"consolidated text {m['celex']} of Regulation (EU) 2023/956 ({m.get('reference') or m['consolidation_date']}), "
            f"retrieved {m['retrieved']} from CELLAR")


def _scope_attribution(s: Store) -> str:
    m = s.annex["meta"]
    return (f"Source: EUR-Lex/CELLAR, consolidated text {m['celex']} of Regulation (EU) 2023/956, "
            f"© European Union, retrieved {m['retrieved']}; table derived by cbam-mcp")


def _values_version(s: Store) -> str:
    m = s.values["meta"]
    return (f"Commission Excel 'Default values definitive period', version {m.get('version')} of "
            f"{m.get('version_date')} ({m.get('version_note')}), file {m.get('filename')!r}, retrieved {m['retrieved']}")


def _values_attribution(s: Store) -> str:
    m = s.values["meta"]
    return (f"Source: European Commission, DG TAXUD, default values Excel version {m.get('version')}, "
            f"CC BY 4.0, retrieved {m['retrieved']}; decimal commas converted to numbers (derived)")


def _cn_version(s: Store, year: int) -> str:
    m = s.cn(year)["meta"]
    return f"CN {year} (EU Vocabularies, {m['scheme']}), bundled subset, retrieved {m['retrieved']}"
...
def _cn_attribution(s: Store, year: int) -> str:
    ...
    return (f"Source: Publications Office of the European Union, EU Vocabularies, Combined Nomenclature {year}, "
            f"European Commission reuse notice, retrieved {m['retrieved']}")
...
def _line(e: dict) -> dict:
    out = {"cn_code": _entry_display(e), "text": e["description"], "ex": bool(e.get("ex")),
           "goods_category": e.get("category"), "greenhouse_gases": e.get("greenhouse_gases")}
    if e.get("except"):
        out["except"] = [f"{format_cn(x['code'])} – {x['description']}" for x in e["except"]]
    if e.get("amended_by"):
        out["amended_by"] = [legal.ACTS.get(c, c) for c in e["amended_by"]]
    return out


def _cn_brief(s: Store, digits: str, year: int = 2026) -> dict:
    cid, c = s.cn_lookup(year, digits[:8])
    if c is None:
    ...
    out = {"version": f"CN {year}", "found": True, "cn_code": format_cn(c[0]), "label": c[1]}
    if c[3] and c[3] != c[1]:
        out["self_explanatory_text"] = c[3]
    return out
...
def _explain(r: dict, digits: str) -> str:
    ...
    if b == "listed":
        e = r["entry"]
        extra = " and none of its exceptions covers it" if e.get("except") else ""
        return f"Annex I lists {_entry_display(e)} – {e['description']}; {code} falls under it{extra}."
    if b == "ex_code":
        e = r["entry"]
        return (f"Annex I lists this as '{_entry_display(e)} – {e['description']}'. 'ex' means only the goods "
                f"that description covers are in scope, not every good under {format_cn(e['code'])}.")
    if b == "excluded_by_exception":
        e, x = r["entry"], r["exception"]
        return (f"Annex I lists {_entry_display(e)} – {e['description']}, except "
                f"{format_cn(x['code'])} – {x['description']}; {code} falls under that exception.")
...
def cbam_scope(cn_code, limit: int = 50) -> dict:
    ...
    out: dict = {"query": cn_code if isinstance(cn_code, str) else str(cn_code), "cn_code": format_cn(digits)}
    ...
    if len(digits) < 8:
        ...
        for code in s.cn_codes_under(2026, digits):
            ...
            item = {"cn_code": format_cn(code), "description": (c[3] or c[1]) if c else ""}
            if cr["status"] == "not_in_scope" and cr["basis"] == "excluded_by_exception":
                item["excluded_by"] = f"{format_cn(cr['exception']['code'])} – {cr['exception']['description']}"
            if cr["status"] == "partially_in_scope" and cr["basis"] == "ex_code":
                item["ex_code"] = _entry_display(cr["entry"])
    ...
    if entry is None and status == "in_scope" and len(r.get("below") or []) == 1:
        entry = r["below"][0]
    if entry:
        out["goods_category"] = entry.get("category")
        out["greenhouse_gases"] = entry.get("greenhouse_gases")
...
def _de_minimis(s: Store, digits: str, entry: dict | None) -> dict | None:
    ...
    if entry is None or not q.get("article_2a_1") or not q.get("annex_vii_1"):
        return None
    m = s.annex["meta"]
    src = f"Article 2a and Annex VII of Regulation (EU) 2023/956, consolidated text {m['celex']}, retrieved {m['retrieved']}"
    code = entry["code"] if entry else digits
    if code.startswith(ELECTRICITY[:4]) or code.startswith(HYDROGEN):
        return {"applies_to_these_goods": False, "article_2a_4": q.get("article_2a_4"), "source": src}
    return {"applies_to_these_goods": True, "article_2a_1": q["article_2a_1"], "annex_vii_point_1": q["annex_vii_1"],
...
def _scope_legal(s: Store) -> dict:
    return {"legally_binding": False,
            ...
            "legal_status_of_data": s.annex["meta"].get("disclaimer"),
...
def _routes(text: str) -> list:
    ...
    for letter in [p.strip("() ") for p in (text or "").split("/") if p.strip("() ")]:
        out.append({"code": letter, "meaning": legal.PRODUCTION_ROUTES.get(letter, "not in the legend")})
    return out


def _number(text: str) -> dict:
    value, kind = parse_value(text)
    out = {"value": value, "as_published": text}
    if kind != "value":
        out["kind"] = kind
    return out
...
def _resolve(s: Store, country) -> dict:
    ...
    if name is None:
        if suggestions:
            raise InputError(f"country {country!r} not recognised; did you mean: {', '.join(suggestions)}?")
        return {"name": str(country).strip(), "kind": "unlisted"}
    if name in s.values["tables"]:
        kind = "other" if name == s.values["other_table"] else "table"
        return {"name": name, "kind": kind, "iso": ISO_BY_TABLE_NAME.get(name)}
    return {"name": name, **extra[name]}
...
def _values_for(s: Store, code: str, origin: dict) -> dict:
    ...
    item = {"table_line": line[1], "description": line[2], "goods_category": line[3]}
    source_table, fallback = origin["name"], None
    row = _row(s, origin["name"], code) if origin["kind"] in ("table", "other") else None
    if origin["kind"] == "unlisted":
        source_table = other
        fallback = {"reason": f"{origin['name']!r} is not one of the countries in the tables",
                    "rule": legal.RULE_NOT_LISTED}
    elif row is None:
        source_table = other
        fallback = {"reason": f"the table for {origin['name']} has no line {line[1]}", "rule": legal.RULE_NO_VALUE}
    elif any(parse_value(x)[1] == "dash" for x in row[:3]):
        source_table = other
        fallback = {"reason": f"the table for {origin['name']} shows '–' for {line[1]}", "rule": legal.RULE_NO_VALUE}
    if fallback:
        row = _row(s, other, code)
        item["values_from_table"] = other
        item["fallback"] = fallback
    else:
        item["values_from_table"] = source_table
    if row is None:
        item["found"] = False
        return item
    item["found"] = True
    direct, indirect, total = _number(row[0]), _number(row[1]), _number(row[2])
    item.update({"total": total["value"], "direct": direct["value"], "indirect": indirect["value"],
                 "as_published": {"direct": row[0], "indirect": row[1], "total": row[2]}})
    if indirect.get("kind") == "not_applicable":
        item["indirect_note"] = "'N/A' in the table"
    if total.get("kind") == "see_below":
    ...
    if a4:
        v, r = (a4.split("|") + [""])[:2]
        item["annex_iv_highest_default"] = {"value": parse_value(v)[0], "as_published": v,
...
def _values_legal(s: Store) -> dict:
    ...
    if check:
        out["checked_against_official_journal"] = (
            f"On {check['checked']} cbam-mcp compared this file with {check['url']}: {check['rows_identical']} of "
            f"{check['rows_compared']} country lines and {check['annex_iv_identical']} of {check['annex_iv_compared']} "
            f"Annex IV lines identical.")
    return out


def _value_notes(s: Store, lines_found: bool) -> list:
    notes = []
    if lines_found:
        notes += [legal.MARKUP_POINTER,
...
def _origin_answer(s: Store, origin: dict) -> dict | None:
    if origin["kind"] == "eu":
        return {"status": "not_a_third_country",
                "explanation": f"{origin['name']} is an EU Member State ({legal.EU_MEMBER_STATES_SOURCE}, checked "
                               f"{legal.CHECKED}). Article 2(1): {s.annex.get('quotes', {}).get('article_2_1', '')}"}
    if origin["kind"] == "annex_iii":
        return {"status": "origin_outside_cbam",
                "explanation": f"{origin['name']} is listed in point 1 of Annex III to Regulation (EU) 2023/956. "
                               f"Article 2(4): {s.annex.get('quotes', {}).get('article_2_4', '')}"}
    return None
...
def default_value(cn_code, country) -> dict:
    ...
    out: dict = {"cn_code": format_cn(digits), "country": origin["name"], "unit": UNIT,
    ...
    if blocked:
        ...
        if not codes:
            ...
            out["status"] = "found"
            if origin["kind"] == "unlisted":
                warnings.append(f"{origin['name']!r} is not a country name in the tables; if it is a third country "
...
def compare_origins(cn_code, countries) -> dict:
    ...
    if isinstance(countries, str):
        countries = [c for c in countries.split(",")]
    ...
    if digits.startswith(ELECTRICITY[:4]):
        ...
        other = {"name": s.values["other_table"], "kind": "other"}
        lines = []
        for code in codes:
            ...
            for o in origins:
                ...
                if blocked:
                    by_country.append({"country": o["name"], **blocked})
                    continue
                v = _values_for(s, code, o)
                by_country.append({k: v.get(k) for k in ("total", "direct", "indirect", "values_from_table", "fallback",
                                                          "production_route", "as_published", "indirect_note")
                                   if v.get(k) is not None} | {"country": o["name"]})
...
def cn_describe(cn_code, year: int = 2026) -> dict:
    ...
    if c is None:
        ...
        out.update({"found": True, "cn_code": format_cn(c[0]), "label": c[1], "indent": c[2]})
        if c[3]:
            out["self_explanatory_text"] = c[3]
        chain, seen, parent = [], set(), c[4]
        while parent and parent not in seen:
            seen.add(parent)
            p = s.cn(year)["concepts"].get(parent)
            if not p:
                break
            chain.append({"cn_code": format_cn(p[0]) if p[0].isdigit() else (p[0] or None), "label": p[1], "indent": p[2]})
            parent = p[4]
        out["hierarchy"] = list(reversed(chain))
        kids = [s.cn(year)["concepts"][k] for k in sorted(s.cn_children(year, cid))]
        if kids:
            out["sub_codes"] = [{"cn_code": format_cn(k[0]) if k[0] else None, "label": k[1]} for k in kids[:100]]
        other_year = 2025 if year == 2026 else 2026
        _, o = s.cn_lookup(other_year, c[0])
        if o is None:
            out[f"in_cn_{other_year}"] = "no such code"
        elif o[1] != c[1]:
            out[f"in_cn_{other_year}"] = {"label": o[1]}
        else:
...
def sources() -> dict:
    ...
    out["datasets"].append({
        ...
        "file": "annex_i.json", "url": a["url"], "version": f"{a['celex']} ({a.get('reference')})",
        "retrieved": a["retrieved"], "sha256": a["sha256"], "rows": {"annex_i": a["annex_i_lines"], "annex_ii": a["annex_ii_lines"]},
        "legally_binding": False, "legal_status": a.get("disclaimer"),
    ...
    out["datasets"].append({
        ...
        "file": "default_values.json", "url": v["url"], "page": v.get("page"), "file_name": v.get("filename"),
        "version": f"{v.get('version')} of {v.get('version_date')}", "version_note": v.get("version_note"),
        "retrieved": v["retrieved"], "sha256": v["sha256"],
        "rows": {"tables": v["tables"], "country_lines": v["rows"], "cn_taric_lines": v["lines"], "annex_iv": v["annex_iv_rows"]},
        "legally_binding": False, "legal_status": legal.EXCEL_NOTICE, "disclaimer_in_file": v.get("disclaimer"),
        "legally_binding_source": legal.VALUES_BINDING_SOURCE, "official_journal_check": v.get("oj_check") and
        {k: v["oj_check"][k] for k in ("checked", "celex", "rows_compared", "rows_identical", "annex_iv_compared",
                                        "annex_iv_identical", "quotes_found")},
    ...
    for year in (2026, 2025):
        m = s.cn(year)["meta"]
        out["datasets"].append({
            "name": f"Combined Nomenclature {year} (subset)", "file": f"cn_{year}.json", "url": m["scheme"],
            "endpoint": m["endpoint"], "dataset": m["dataset"], "subset": m["subset"], "retrieved": m["retrieved"],
            "sha256_of_responses": m["sha256_of_responses"], "rows": {"concepts": m["concepts"]},
            "legally_binding": False, "legally_binding_source": legal.CN_BINDING_SOURCE[year],
            "licence": legal.LICENCES["cn"], "attribution": _cn_attribution(s, year)})
    later = s.dir / "later_acts.json"
    try:
        out["later_acts"] = json.loads(later.read_text(encoding="utf-8"))
