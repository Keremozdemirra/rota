"""mcp_upkeep.py against fixture configs and canned registry answers. No network, no real home directory."""
...
class Resolve(Isolated):
    def test_npx_scoped_unpinned(self):
        r = mcp_upkeep.resolve(entry("npx", ["-y", "@modelcontextprotocol/server-filesystem", "/tmp"]))
    ...
    def test_npx_pinned_and_latest(self):
        self.assertTrue(mcp_upkeep.resolve(entry("npx", ["-y", "pkg@1.2.3"]))["pinned"])
        self.assertFalse(mcp_upkeep.resolve(entry("npx", ["pkg@latest"]))["pinned"])

    def test_npx_package_flag(self):
        r = mcp_upkeep.resolve(entry("npx", ["-y", "-p", "@scope/pkg@2.0.0", "bin-name"]))
        self.assertEqual((r["package"], r["version"]), ("@scope/pkg", "2.0.0"))

    def test_windows_npx_cmd(self):
        self.assertEqual(mcp_upkeep.resolve(entry("C:\\nodejs\\npx.cmd", ["-y", "pkg"]))["kind"], "npm")

    def test_pnpm_dlx(self):
        self.assertEqual(mcp_upkeep.resolve(entry("pnpm", ["dlx", "pkg"]))["package"], "pkg")

    def test_uvx(self):
        r = mcp_upkeep.resolve(entry("uvx", ["mcp-server-fetch==2026.8.18"]))
    ...
    def test_uvx_from_and_extras(self):
        r = mcp_upkeep.resolve(entry("uvx", ["--from", "pkg[cli]", "pkg-server"]))
        self.assertEqual((r["package"], r["pinned"]), ("pkg", False))

    def test_uv_tool_run_git(self):
        r = mcp_upkeep.resolve(entry("uv", ["tool", "run", "--from", "git+https://github.com/o/n", "x"]))
        self.assertEqual((r["kind"], r["repo"], r["pinned"]), ("git", "o/n", False))

    def test_pipx_run(self):
        self.assertEqual(mcp_upkeep.resolve(entry("pipx", ["run", "pkg"]))["package"], "pkg")

    def test_docker_skips_flag_values(self):
        r = mcp_upkeep.resolve(entry("docker", ["run", "-i", "--rm", "-e", "TOKEN", "-v", "/a:/b",
        ...
        self.assertFalse(mcp_upkeep.resolve(entry("docker", ["run", "mcp/fetch"]))["pinned"])
        digest = "mcp/fetch@sha256:" + "a" * 64
        self.assertTrue(mcp_upkeep.resolve(entry("docker", ["run", digest]))["pinned"])

    def test_docker_registry_port_is_not_a_tag(self):
        self.assertFalse(mcp_upkeep.resolve(entry("docker", ["run", "localhost:5000/img"]))["pinned"])

    def test_remote(self):
        r = mcp_upkeep.resolve(entry(url="https://mcp.example.com/mcp"))
    ...
    def test_local_checkout(self):
        ...
        r = mcp_upkeep.resolve(entry("python3", [str(d / "src" / "server.py")]))
        self.assertEqual((r["kind"], r["repo"]), ("local", "o/n"))

    def test_unknown(self):
        self.assertEqual(mcp_upkeep.resolve(entry("some-binary"))["kind"], "unknown")
...
class Grammar(Isolated):
    def test_npm_names(self):
        ...
        for s in ok:
            self.assertEqual(mcp_upkeep.npm_spec(s)["kind"], "npm", s)
        for s in bad:
            self.assertNotEqual(mcp_upkeep.npm_spec(s).get("kind"), "npm", s)

    def test_pypi_names(self):
        for s in ["mcp-server-fetch", "a", "A.b_c-1"]:
            self.assertEqual(mcp_upkeep.py_spec(s)["kind"], "pypi", s)
        for s in ["-x", "x-", "/abs", "https://x/y", "a b", ""]:
            self.assertNotEqual(mcp_upkeep.py_spec(s).get("kind"), "pypi", s)

    def test_github_host_is_parsed_not_searched(self):
        self.assertEqual(mcp_upkeep.github_ref("git+https://tok:x@github.com/o/n.git#main")[:2], ("o/n", "main"))
        self.assertEqual(mcp_upkeep.github_ref("git@github.com:o/n.git")[0], "o/n")
        self.assertEqual(mcp_upkeep.github_ref("https://github.com/o/n/tree/main/src/fetch")[0], "o/n")
        for s in ["https://evil.example/github.com/o/n", "https://github.com.evil.example/o/n",
            ...
            self.assertIsNone(mcp_upkeep.github_ref(s), s)


class Masking(Isolated):
    def test_urls(self):
        self.assertEqual(mcp_upkeep.mask_url("https://alice:hunter2@mcp.internal.example/sse"),
                         "https://***@mcp.internal.example/sse")
        self.assertEqual(mcp_upkeep.mask_url("https://server.smithery.ai/@x/y/mcp?api_key=sk-SMITHERYSECRET"),
                         "https://server.smithery.ai/@x/y/mcp?***")
        self.assertEqual(mcp_upkeep.mask_url("https://mcp.example.com/s/Zx81kPq0Lm4Nv7Rt2Wy5Bc9D/mcp"),
                         "https://mcp.example.com/s/***/mcp")

    def test_args(self):
        got = mcp_upkeep.mask_args(["--api-key=sk-ARGSECRET", "--token", "abc123", "KEY=value", "pkg==1.0",
    ...
    def test_strict_hides_every_value(self):
        self.assertEqual(mcp_upkeep.mask_args(["--port", "8080", "s3cret", "--x=y"], strict=True),
                         ["--port", "***", "***", "--x=***"])

    def test_remote_text_is_marked_and_cut(self):
        t = mcp_upkeep.remote_text("a\x1b[31mb\u202e\nc>> " + "x" * 400)
...
class Discover(Isolated):
    def test_reads_names_commands_never_env(self):
        ...
        servers, searched = mcp_upkeep.discover(self.home, self.cwd, [])
...
class Examine(Isolated):
    def test_active_licensed_pinned_is_clean(self):
        ...
        r = mcp_upkeep.examine(entry("npx", ["@o/server@1.0.0"]), net, TODAY)
    ...
    def test_abandoned_unlicensed_deprecated(self):
        ...
        r = mcp_upkeep.examine(entry("npx", ["-y", "@o/server"]), net, TODAY)
    ...
    def test_archived(self):
        ...
        self.assertEqual(mcp_upkeep.examine(entry("npx", ["@o/server@1"]), net, TODAY)["flags"][0], "archived")

    def test_repo_gone(self):
        net = FakeNet({NPM: npm_doc()})  # GitHub answers 404
        self.assertIn("repository missing", mcp_upkeep.examine(entry("npx", ["@o/server@1"]), net, TODAY)["flags"])
    ...
    def test_rate_limit_falls_back_to_census(self):
        ...
        r = mcp_upkeep.examine(entry("npx", ["@o/server@1"]), net, TODAY)
    ...
    def test_offline_reports_config_facts_only(self):
        r = mcp_upkeep.examine(entry("npx", ["-y", "@o/server"]), None, TODAY)
    ...
    def test_pypi_repo_from_project_urls(self):
        ...
        self.assertEqual(mcp_upkeep.examine(entry("uvx", ["p==1"]), net, TODAY)["repo"], "o/n")
    ...
    def test_census_from_the_published_file(self):
        ...
        net = FakeNet({"https://registry.npmjs.org/@21st-dev%2Fmagic": fixture("npm-21st-dev-magic.json"),
                       "https://api.github.com/repos/21st-dev/magic-mcp": 403, mcp_upkeep.CENSUS: census})
        r = mcp_upkeep.examine(entry("npx", ["-y", "@21st-dev/magic@0.2.3"]), net, TODAY)
...
class Output(Isolated):
    def test_strict_exit_code_and_markdown(self):
        ...
        md = mcp_upkeep.render_markdown([mcp_upkeep.examine(entry("npx", ["pkg"]) | {"name": "x|y"}, None, TODAY)], TODAY)
    ...
    def test_github_and_census_both_unreachable(self):
        self.serve({"https://registry.npmjs.org/@21st-dev%2Fmagic": fixture("npm-21st-dev-magic.json"),
                    "https://api.github.com/repos/21st-dev/magic-mcp": 403, mcp_upkeep.CENSUS: 503})
